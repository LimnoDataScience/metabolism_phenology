    ...
    for (id.z in unique(floor(unique(obs$depth)))){
      ...
      if (!is.na(id.end)){
        if ( flag == 0){
          ...
          SED <- mean(sim$Fsed[match(obs.z$sampledate[id.start],as.Date(sim$datetime)) : match(obs.z$sampledate[id.end],as.Date(sim$datetime))])
          
        } else {
          NEP <- mean(sim$Fmineral[match(obs.z$sampledate[id.start],as.Date(sim$datetime)) : match(id.end,as.Date(sim$datetime))])
          SED <- mean(sim$Fsed[match(obs.z$sampledate[id.start],as.Date(sim$datetime)) : match(id.end,as.Date(sim$datetime))])
...
ggplot(coeff ) +
  ...
  geom_density(aes(SED + NEP,  fill = 'ODEM'), alpha = 0.1) +
  ...
  ggplot(coeff ) +
    geom_point(aes(Jz, SED + NEP, col = id,)) +
    ggtitle('Total average DO depletion') +
    xlab('average Jz (Livingstone)')+ylab('average SED+NEP (g/m3/d)')
  # geom_point(aes(Jv, NEP, col = as.factor(year))) +
  # ylim(min(c(coeff$Jv, coeff$NEP), na.rm = T),max(c(coeff$Jv, coeff$NEP), na.rm = T)) +
  # xlim(min(c(coeff$Jv, coeff$NEP), na.rm = T),max(c(coeff$Jv, coeff$NEP), na.rm = T)) +
  theme_bw()
