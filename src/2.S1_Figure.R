...
df.long = df %>% 
  ...
  mutate(name = fct_relevel(name,'Concave','Linear','Convex'))#fct_relevel(name,'Concave','Linear','Convex','Hypoxic','Anoxic')) 

# Cluster lables 
cluster.labels = NA
# order = match(lakeinv, c('Hypoxic','Linear','Convex')) # Order in the same way as 
order = match(lakeinv, c('Concave','Linear','Convex'))#c('Concave','Linear','Convex','Hypoxic','Anoxic')) # Order in the same way as
for (i in 1:3){
  j = order[i]
...
g.cluster = ggplot(df.long) +
  ...
  scale_color_manual(values = c('lightblue3','gold','red4','red1','red4'), name = 'Cluster',
...
g1 <- ggplot(m.df.grd, aes(x = variable, y = lake, fill = as.factor(value))) + 
  scale_fill_manual(values = c('lightblue3','gold','red4','red1','red4'), name = 'Cluster', 
