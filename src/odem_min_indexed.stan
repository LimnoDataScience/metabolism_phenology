  ...
  { // create a block so discreate values can be created that aren't treated as estimated parameters
	...
	sed2[i] = SED2[i_Param[i]] *  (fmax((DO_tot[i-1]*tau+mu),1e-06)/(khalf + fmax((DO_tot[i-1]*tau+mu),1e-06))) * theta0[i-1] / mean_depth;
        ...
        if(first_day == 1){
        	...
        	nu[i] = k600[i-1] *  (x_eq[i-1] - DO_epi[i-1]) / tddepth[i-1]; //if DO is negative...use 0.1 instead of inferred DO
        	DO_epi[i] =  ((DO_epi[i-1] + nep[i] + nu[i])*volume_epi[i-1] + (delvol[i]*x_do1))/volume_epi[i];
        }                                               
	...
	sed2[i] = (SED2[i_Param[i]] *  (fmax((DO_hyp[i-1]*tau+mu),1e-06)/(khalf + fmax((DO_hyp[i-1]*tau+mu),1e-06))) * theta2[i-1]) / fmax((volume_hyp[i-1]/area_hyp[i-1]),1);
    	    DO_hyp[i] =  ((DO_hyp[i-1] - sed2[i] + mineral[i])*volume_hyp[i-1] - (delvol[i]*x_do1))/volume_hyp[i];
    	}
