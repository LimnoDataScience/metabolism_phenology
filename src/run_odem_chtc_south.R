...
dummyinput$nep_lim = 2000;
dummyinput$sed_lim = 4000;
dummyinput$smooth_sigma=0.1;#0.1;
dummyinput$max_depth = dummyinput$max.d;
...
sm <- stan_model(file = "odem_min_indexed.stan")
...
# saveRDS(fit, file = "fit_Allequash.rds")
...
#          index = strsplit(var, "\\[|\\]|,") %>% map_int(~as.integer(.x[2])))
# 
# outvars <- c("o2_epi","o2_hyp","fatm","fnep","fsed2","fentr_epi","fentr_hyp","NEP_mgm3d","SED_mgm2d")
# 
...
# odem_stan <- cbind(in5yr[,1],odem_stan,simdata)
# odem_stan <- odem_stan %>% 
...
#          SED = SED_mgm2d_mean)
# 
# save(odem_stan, file = paste0('Allequash.Rda'))
