...
# Read in files
seasonal.df = read_csv('Processed_Output/NEP_seasonal-epi.csv')
cum.seasonal.df = read_csv('Processed_Output/NEP_seasonal_cumulative-epi.csv')

seasonal.df$id <- factor(seasonal.df$id , levels= (c("Allequash","BigMuskellunge","Crystal","Sparkling", "Trout","Fish","Mendota","Monona")))

seasonal.df2 = read_csv('Processed_Output/NEP_seasonal-hyp.csv')
cum.seasonal.df2 = read_csv('Processed_Output/NEP_seasonal_cumulative-hyp.csv')

seasonal.df2$id <- factor(seasonal.df$id , levels= (c("Allequash","BigMuskellunge","Crystal","Sparkling", "Trout","Fish","Mendota","Monona")))
...
g1 = ggplot(seasonal.df[!is.na(seasonal.df$movavg),], 
  ...
  ylab(expression(atop("Filtered seasonal NEP,epi +"," long-term average [g DO"*~m^{-2}*""*~d^{-1}*"]"))) +
...
g3 = ggplot(seasonal.df2[!is.na(seasonal.df$movavg),], 
            aes(as.Date(yday, origin = as.Date('2019-01-01')), (movavg) + NEP_avg2, col = id)) + #*volume/area/1000
  geom_ribbon(aes(ymin = movavg_min + NEP_avg2, ymax = movavg_max + NEP_avg2, col = id, fill = id), size = 0.3, alpha = 0.03) +
  geom_line(aes(linetype=id, col = id), size = 1.5) +
  # ylab(expression("7-day mov. average of seasonal decomposed total flux rate [g DO"*~m^{-2}*""*~d^{-1}*"]")) +
  ylab(expression(atop("Filtered seasonal (NEP,hypo - SED) +"," long-term average [g DO"*~m^{-2}*""*~d^{-1}*"]"))) +
  # xlab('Day of the year') +
  scale_x_date(breaks = '2 months', labels = date_format("%b")) +
  scale_color_brewer(palette="Dark2") +
  scale_linetype_manual(values = c(1,1,1,1,1,2,2,2))+
  ...
  theme(axis.title.x = element_blank(),
        ...
        axis.text.y= element_text(size = 20), text = element_text(size = 15), legend.title = element_blank(), strip.text =element_text(size = 20),
        legend.position = 'bottom')+
  guides(); g3#linetype = FALSE


g4 = g1 / g3 + plot_annotation(tag_levels = 'A') + plot_layout(guides = 'collect') &
  theme(legend.position='bottom'); g4

ggsave(file = 'Figures/Fig_5-others.png', g4, dpi = 300, width =300, height = 200, units='mm')
...
ggsave(file = 'Figures/Fig_5-hypo.png', g1, dpi = 300, width =300, height = 150, units='mm')
