...
g.param <- ggplot(subset(j.m, year > 1995 & year < 1997)) + 
  ...
  geom_line(aes(x=datetime, y=(-1) * (nSED)*2, col = 'Xsed')) +
  geom_ribbon(aes(x=datetime, ymin=-1 * nSED_min*2, ymax= -1 *nSED_max*2, col = 'Xsed'), linetype =2,alpha=0.2) +
...
g.conc <- ggplot(subset(lake.odem, year > 1995 & year < 1997)) + 
