...
weeklyInterpolate.1D <- function(obs, varname) {
  full_x = seq.Date(from = range(obs$sampledate,na.rm = T)[1],to = range(obs$sampledate,na.rm = T)[2], by = 'week')
  yout = approx(x = obs$sampledate, y = pull(obs[,varname]), xout = full_x)
  output = data.frame(date = full_x, var = yout$y)
  
  return(output)
}

monthlyInterpolate.1D <- function(obs, varname) {
  full_x = seq.Date(from = range(obs$sampledate,na.rm = T)[1],to = range(obs$sampledate,na.rm = T)[2], by = 'month')
  yout = approx(x = obs$sampledate, y = pull(obs[,varname]), xout = full_x)
  output = data.frame(date = full_x, var = yout$y)
...
decompose.monthly <- function(df.monthly, lakeAbr, var) {
  
  df.monthly = df.monthly %>% mutate(year = year(date), month = month(date))
  ## weights for moving avg
  fltr <- c(1/2, rep(1, times = 11), 1/2)/12
  ## create a time series (ts) object from the var data
  var.load <- ts(data = df.monthly$var, frequency = 12, start = c(df.monthly$year[1],
                                                                 df.monthly$month[1]))
  ## estimate of trend
  ...
  pY = ggplot(output) + geom_line(aes(x = date, y = value), color = 'red4') +
    ...
    theme_bw()
    # labs(title = paste0('Timeseries decomposition: Lake ',lakeAbr,', var: ',var))
